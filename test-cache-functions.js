...
// Test configuration
const TEST_CONFIG = {
  // Test with a known artist that should exist in the database
  testArtistUrlKey: 'playboi-carti', // You can change this to any artist in your database
  maxSongsToTest: 3, // Limit to avoid long test times
  verbose: true
};
...
class CacheFunctionTester {
  ...
  async testPopulateArtistSongs() {
    ...
    try {
      ...
      if (result.data.success) {
        ...
        this.log(`New songs: ${result.data.newSongs}`);
        this.log(`Fully cached: ${result.data.isFullyCached}`);
  ...
  async testScrapeSongLyrics() {
    ...
    try {
      // First get artist info to find song IDs
  ...
  async testLoadStartingFromId() {
    ...
    try {
      ...
      if (result.data.success) {
        ...
        this.log(`Loaded songs: ${Object.keys(result.data.loadedSongs).length}`);
  ...
  async runAllTests() {
    this.log('🚀 Starting Cache Function Tests');
...
// Helper function to find a good test artist
async function findTestArtist() {
  try {
    console.log('🔍 Finding a suitable test artist...');
    
    // Get a few artists from the database
    const artistsQuery = query(collection(db, 'artists'), limit(10));
    const snapshot = await getDocs(artistsQuery);
    
    if (snapshot.empty) {
      throw new Error('No artists found in database. Please run upload-to-firestore.js first.');
    }
    
    // Look for an artist with a geniusId
    for (const doc of snapshot.docs) {
      const data = doc.data();
      if (data.geniusId && data.type === 'popular') {
        console.log(`✅ Found test artist: ${data.name} (${doc.id})`);
        return doc.id;
      }
    }
    
    // If no popular artist with geniusId, use the first one with geniusId
    for (const doc of snapshot.docs) {
      const data = doc.data();
      if (data.geniusId) {
        console.log(`✅ Using test artist: ${data.name} (${doc.id})`);
        return doc.id;
      }
    }
    
    throw new Error('No artists with Genius IDs found in database');
    
  } catch (error) {
    console.error('Error finding test artist:', error);
    throw error;
  }
}
...
async function main() {
  try {
    // Find a test artist if the default one doesn't work
    if (TEST_CONFIG.testArtistUrlKey === 'playboi-carti') {
      try {
        const artistDoc = await getDoc(doc(db, 'artists', TEST_CONFIG.testArtistUrlKey));
        if (!artistDoc.exists()) {
          console.log('Default test artist not found, searching for alternative...');
          TEST_CONFIG.testArtistUrlKey = await findTestArtist();
        }
      } catch (error) {
        console.log('Searching for test artist in database...');
        TEST_CONFIG.testArtistUrlKey = await findTestArtist();
      }
    }
...
export { CacheFunctionTester, findTestArtist }; 